    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative pt-24 pb-12 md:pt-40 md:pb-24 overflow-hidden px-6">
        <div className="container mx-auto relative z-10">
          <div className="flex flex-col gap-6 md:gap-8 items-start">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full glass border-primary/20 text-primary text-[10px] font-black uppercase tracking-widest animate-pulse">
            ...
            <div className="space-y-2 md:space-y-4">
              ...
              <p className="text-muted-foreground text-xs md:text-base font-medium leading-relaxed max-w-xs md:max-w-md">
                AI-driven diagnostics and professional repair protocols for your hardware.
              </p>
            </div>
            
            <div className="grid grid-cols-2 gap-3 w-full max-w-sm">
              <Link href="/guides" className="w-full">
                <Button className="w-full rounded-2xl h-14 md:h-16 font-black uppercase tracking-widest text-[10px] bg-primary text-primary-foreground shadow-lg shadow-primary/20 active:scale-95 transition-all">
                  Initialize
                </Button>
              </Link>
              <Link href="/troubleshoot" className="w-full">
                <Button variant="outline" className="w-full rounded-2xl h-14 md:h-16 font-black uppercase tracking-widest text-[10px] border-white/10 glass active:scale-95 transition-all gap-2">
      ...
      <section className="container mx-auto px-6 mb-12">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Active Systems</h2>
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
          {REPAIR_CATEGORIES.map((cat) => (
            <Link key={cat.name} href={`/guides?category=${cat.name.toLowerCase()}`}>
              <div className="glass-card p-4 md:p-6 rounded-2xl md:rounded-[2.5rem] flex flex-col items-center gap-2 transition-all active:scale-90">
                <div className="w-10 h-10 md:w-16 md:h-16 bg-white/5 rounded-xl md:rounded-2xl flex items-center justify-center text-primary shadow-inner">
                  <CategoryIcon name={cat.icon} className="w-5 h-5 md:w-8 md:h-8" />
                </div>
                <span className="font-bold text-[8px] md:text-xs uppercase tracking-tighter truncate w-full text-center">{cat.name}</span>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section className="container mx-auto px-6 pb-24 md:pb-32">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground/60">Priority Protocols</h2>
