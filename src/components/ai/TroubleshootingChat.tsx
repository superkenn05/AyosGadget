...
export default function TroubleshootingChat() {
  ...
  return (
    <Card className="w-full max-w-5xl mx-auto glass border-black/5 dark:border-white/10 rounded-[2.5rem] md:rounded-[4rem] overflow-hidden flex flex-col shadow-2xl relative bg-card/40">
      <div className="absolute inset-0 scan-line pointer-events-none" />
      
      <CardHeader className="p-6 md:p-12 border-b border-black/5 dark:border-white/5 bg-primary/[0.02] relative">
      ...
      <CardContent className="p-0 flex-grow relative overflow-hidden bg-muted/5 dark:bg-muted/5">
            ...
            {messages.map((msg, i) => (
              ...
              >
                ...
                <div className={`p-6 md:p-10 rounded-[2rem] md:rounded-[3.5rem] max-w-[90%] md:max-w-[85%] text-sm md:text-lg leading-relaxed font-medium shadow-sm relative overflow-hidden ${
                  msg.role === 'user' 
                    ? 'bg-primary text-primary-foreground rounded-tr-none border border-black/10 dark:border-white/10' 
                    : 'glass text-foreground rounded-tl-none border-l-4 border-l-primary'
                }`}>
                  {msg.role === 'model' && (
                    <div className="absolute top-4 right-6 opacity-10 dark:opacity-20">
        ...
        <div className="flex w-full gap-4 md:gap-8 items-center">
          <div className="relative flex-grow group">
            <Input
              ...
              className="rounded-full border-black/10 dark:border-white/10 bg-white dark:bg-white/5 focus:border-primary/50 h-16 md:h-24 pl-8 md:pl-12 pr-24 md:pr-32 text-lg md:text-2xl font-bold glass tracking-tight placeholder:text-muted-foreground uppercase text-foreground"
            />
            <Button 
              onClick={handleSend} 
              disabled={isLoading}
